...
from mcrand import sample  # noqa 402
...
current_milli_time = lambda: time.time() * 1000  # noqa 731


def generate_distribution_plot(title, subtitle, numpy_rand, f, x0, xf, N, *args):
    ...
    plt.plot(x, exponential(x), color='r', label='PDF')
    plt.text(2, 0.95, subtitle, size=15)
...
def gaussian_test():
    ...
    t0 = current_milli_time()
    rand = sample(gaussian, x0, xf, N, mu, sigma)
    tf = current_milli_time()
...
def cauchy_test():
    ...
    t0 = current_milli_time()
    rand = sample(cauchy, x0, xf, N, x0_cauchy, gamma)
    tf = current_milli_time()
    ...
    plt.plot(x, cauchy(x, x0_cauchy, gamma), color='r', label=r'Cauchy PDF $γ=%.2f$, $x_0=%.2f$' % (gamma, x0_cauchy))
...
def symmetric_maxwell_boltzmann_test():
    ...
    plt.text(-10.5, 0.135, r'$PDF(x)=\sqrt{\frac{1}{2\pi}}\cdot\frac{x^2\exp(-\frac{x^2}{2\sigma^2})}{\sigma^3}$',
             size=15)
